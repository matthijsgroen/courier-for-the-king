...
g.defineLocation("river", ({ describe, onLeave, interaction }) => {
  describe(() => {
    g.onState(g.item("treasureHunt").hasFlag("active"), () => {
  ...
  interaction(
    ...
    () => {
      g.onState(g.character("horse").hasFlag("cart"), () => {
        g.text(
          "You want to bring {b}[characters.horse.name]{/b} to the waterfront, luckily the slope is gentle so the carriage can follow along."
        );
      }).else(() => {
        g.text(
