...
g.defineLocation(
  "darkwoods",
  ({ describe, interaction, onLeave, onEnter, hasFlag }) => {
    describe(() => {
      g.text(
      ...
      g.onState(hasFlag("visited"), () => {
        ...
        g.onState(g.character("dragon").hasState("known"), () => {
          ...
          g.character("player").say(
            "Brr... I have {b}no reason{/b} to be here."
          );
        });
      });
    });
    ...
    onEnter("tower", () => {
      g.onState(g.character("horse").hasState("following"), () => {
        g.text(
          "You untie [characters.horse.name], and together you walk back into the dark woods."
        );
      }).else(() => {
        g.text("You walk back into the dark woods.");
      });
    });
